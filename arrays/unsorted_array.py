...
class UnsortedArray:
    ...
    def min_in_array(self):
        ...
        for index in range(0, len(self._array)):
            if self._array[index] < self._array[indexs]:
                self._array[indexs] = self._array[index]
                indexs = index
        return f"Min index: {indexs}, value: {self._array[indexs]}"

    def max_and_min_in_array(self):
        if len(self._array) == 0:
            raise Exception("The array is empty")
        max_index = 0
        min_index = 0
        for index in range(0, len(self._array)):
            if self._array[index] > self._array[max_index]:
                max_index = index
            elif self._array[index] < self._array[min_index]:
                min_index = index
        return (f"Max value: {self._array[max_index]}, index: {max_index}, \n"
                f"Min value: {self._array[min_index]}, index: {min_index}")
